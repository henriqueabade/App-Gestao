...
// showToast util disponível globalmente em utils/notifications.js
...
function showInfoPopup(target, item) {
    hideInfoPopup();
    const popup = showPopup(target, createPopupContent(item));
    window.electronAPI?.log?.(`showInfoPopup left=${popup.style.left} top=${popup.style.top} id=${item.id}`);
