(function(){
  ...
  const openReplaceModal = async index => {
    ...
    const context = {
      ...
      rows,
      listaProdutos,
      getListaProdutos: () => listaProdutos,
      setListaProdutos: value => {
    ...
    window.OrcamentoReplaceModalContext = context;
    await Modal.open('modals/orcamentos/substituir-peca.html', '../js/modals/orcamento-substituir-peca.js', 'converterReplaceModal', true);
    if (typeof Modal?.waitForReady === 'function') {
      await Modal.waitForReady('converterReplaceModal');
    }
  };
...
// Botões básicos
  function handleCancelConversion() {
    try { Modal.close('converterReplaceModal'); }
