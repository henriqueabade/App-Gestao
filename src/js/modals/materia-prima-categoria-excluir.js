;(function(){
  ...
  let confirm = false;
  aoConfirmar(document.getElementById('excluirCategoria'), async () => {
    const nome = select.value;
    if (!nome) return;
    if (!confirm) {
      try {
        const dependente = await window.electronAPI.verificarDependenciaCategoria(nome);
        if (dependente) {
          confirmTxt.textContent = 'Não é possível excluir esta categoria pois existem itens registrados.';
