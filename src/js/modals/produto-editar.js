  ...
  function init(){
    ...
    regraInfoBtn?.addEventListener('click', () => {
      ...
      window.DialogPadrao?.info({
        ...
        ...regra.caixa({ codigo: codigoInput?.value.trim() || produtoSelecionado.codigo, nome: nomeInput?.value.trim() || produtoSelecionado.nome })
      });
    });
    ...
    function renderItens(data){
      ...
      ordenados.forEach(([proc, arr]) => {
        ...
        arr.forEach(item => {
          ...
          tr.innerHTML = `
            <td data-perm-col="col_ins_mp" class="py-3 px-2 text-sm text-white">${item.nome ?? '—'}</td>
            <td data-perm-col="col_ins_qtd" class="py-3 px-2 text-sm text-left quantidade-cell"><span class="quantidade-text">${formatNumber(item.quantidade)}</span></td>
            <td data-perm-col="col_ins_unidade" class="py-3 px-2 text-sm text-left unidade-cell">${item.unidade ?? '—'}</td>
            <td data-perm-col="col_ins_custo_un" class="py-3 px-2 text-sm text-left text-white item-unit">${formatCurrency(item.preco_unitario || 0)}</td>
            <td data-perm-col="col_ins_custo_total" class="py-3 px-2 text-sm text-left text-white item-total">${formatCurrency((item.preco_unitario || 0) * (item.quantidade || 0))}</td>
     ...
     * processo antigo aparece dentro do grupo dele mas carregava um `ordem`
     * maior que o dos processos seguintes — e era esse número que ia para o
     * banco. É a mesma renumeração que o arrastar-e-soltar já fazia.
     */
    function sincronizarOrdemComTabela(){
      Array.from(tableBody.querySelectorAll('tr.item-row')).forEach((linha, idx) => {
        ...
        if(!item) return;
        const novaOrdem = idx + 1;
        if(item.ordem === novaOrdem) return;
