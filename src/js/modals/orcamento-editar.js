 (async () => {
    ...
    if(editarCondicao.value==='vista'){
      pagamentoBox.innerHTML=`
        <div class="relative w-40">
          <input id="editarPrazoVista" type="number" min="0" data-numeric-decimals="0" placeholder=" " class="peer w-full ctl-campo bg-input border border-inputBorder text-white placeholder-transparent focus:border-primary focus:ring-2 focus:ring-primary/50 transition" data-filled="false" />
          <label for="editarPrazoVista" class="absolute left-3 top-1/2 -translate-y-1/2 text-sm text-gray-300 pointer-events-none transition-all duration-150 peer-placeholder-shown:top-1/2 peer-placeholder-shown:-translate-y-1/2 peer-focus:top-0 peer-focus:-translate-y-full peer-focus:text-xs peer-focus:text-primary peer-valid:top-0 peer-valid:-translate-y-full peer-valid:text-xs peer-data-[filled=true]:top-0 peer-data-[filled=true]:-translate-y-full peer-data-[filled=true]:text-xs">Prazo (dias)</label>
        </div>`;
  ...
  if (daProspeccao) {
    ...
    transportadoraTexto.className = editarTransportadora.className
      .replace('appearance-none', '').replace('ctl-campo--icone', '');
