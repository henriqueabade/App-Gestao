...
test('produção: acumulado por item e setor, status e valor das pendentes', () => {
  const eventos = [
    { id: 1, pedido_id: 55, pedido_item_id: 501, produto_id: 10, setor_id: 2, quantidade: 3, data_finalizacao: '2026-08-15', competencia: '2026-08', status: 'ativo' },
    { id: 2, pedido_id: 55, pedido_item_id: 501, produto_id: 10, setor_id: 2, quantidade: 1, data_finalizacao: '2026-08-16', competencia: '2026-08', status: 'estornado' },
    { id: 3, pedido_id: 55, pedido_item_id: 501, produto_id: 10, setor_id: 2, quantidade: -3, data_finalizacao: '2026-09-02', competencia: '2026-09', status: 'ativo', estorno_de: 1 }
  ];
  ...
  const fechamento = { id: 1, tipo: 'producao', competencia: '2026-08', status: 'fechado', total: 75, por_setor: JSON.stringify([{ setor_id: 2, setor: 'Pintura', pecas: 3, total: 75 }]) };
  const itens = [{ id: 9, fechamento_id: 1, tipo_item: 'producao', producao_evento_id: 1, quantidade: 3, valor_unitario: '25.00', total: '75.00', setor: 'Pintura', produto: 'POL-01 — Poltrona', detalhes: '{}' }];
  const estado = estadoDosFechamentos({ fechamentos: [fechamento], itens, pagamentos: [], tipo: 'producao' });
  const pend = producao.pendentes({
    eventos, estado, valores: [{ id: 1, setor_id: 2, produto_id: null, valor_unitario: '40', ativo: true }],
    itensPor: new Map([['501', { id: 501, pedido_id: 55, codigo: 'POL-01', nome: 'Poltrona', quantidade: 5 }]]),
    setoresPor: new Map([['2', { id: 2, nome: 'Pintura' }]]), pedidosPor: new Map([['55', { id: 55, numero: '2548' }]])
  });
  assert.equal(pend.length, 1, 'o fechado e o estornado não ficam pendentes');
  assert.equal(pend[0].valor_unitario, 25, 'o estorno usa o valor com que o original foi fechado, não o de hoje (40)');
  assert.equal(pend[0].total, -75);
  ...
  assert.deepEqual(saldos.map(s => [s.setor, s.total, s.competencia]), [['Pintura', -75, '2026-10']]);
