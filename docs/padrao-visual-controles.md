...
| 15 | Financeiro | **feito em 22/09** — tela: só o cabeçalho encostou no padrão (seletor de competência, Hoje, engrenagens, Conciliar BB e Atualizar de 44 → 40 px, peso 600, cantos 11,2; a regra de 2,75rem que igualava tudo à mão saiu); cartões, painéis e letras da tela ficaram (são a referência). Os 20 modais foram do tamanho grande para o padrão: títulos, rodapés, campos, o seletor de competência dos filtros, os títulos de seção das configurações (`.fin-cfg-secao__titulo` com as medidas do `ctl-secao`), cartões de opção e o segmento Visualizar/PDF/Excel (14 px, 40 px), a lista de dados só de leitura (13/14 px; o total em destaque ficou). No JS: as caixas de motivo (estorno), "Confirmar peça" e, no cartão da peça, o campo de unidades pequeno com Tudo/Nada/"Tudo pronto" no botão pequeno. Ações dentro das linhas das tabelas ficaram. O conferidor de JS passou a ler `criar('button', …)`. Commitado direto. |
| 16 | Relatórios | **feito em 22/09** — tela inteira (cabeçalho, os 8 filtros com os falsos selects de país/estado, barra de resultados) e as caixas "Salvar modelo" e "Agendar" (na mesma página). As abas de categoria e de visão (Tabela/Gráficos/Master-Detail) viraram botão de barra (32 px, 13 px). A folha tinha regras de duas classes segurando recuo e cantos dos campos e o tamanho dos botões do filtro: saíram. "Filtros"/"Indicadores" na letra do `ctl-secao-tela`. As setinhas `text-xs` dentro dos botões de lista ficam pequenas (controles.css exclui `i.text-xs`). Títulos de gráfico e o nome do registro no Master-Detail ficaram. Commitado direto. |
| 17 | Configurações | **feito em 22/09** — tela única (sem modais). Os botões próprios "Salvar alterações"/"Cancelar" (`personal-button`, em pílula de 46 px e letra de 15,2) e o "Atualizar foto" passaram para 40 px, 14 px/600 e os cantos do padrão (as cores ficaram); campos com `ctl-campo` (a folha segurava recuo e cantos), rótulos 13 px, títulos dos cartões de 20 → 16,8 px (a letra do `ctl-secao-tela`). Commitado direto. |
| — | Globais: caixa de diálogo (`DialogPadrao`) | **feito em 22/09** — os botões do rodapé (`.dlg-rodape button`, em dialogo-padrao.css) passaram de ~38 px e cantos de 9,6 px para as medidas do padrão (40 px, 14 px/600, cantos 11,2), pela própria regra, sem classe: vale para o DialogPadrao, o resultado da importação de planilha (`AcoesCsv`) e o "Não é possível excluir"/"Insumo já cadastrado" de Matéria-prima. O texto principal da caixa (`.dlg-lead`) de 14,4 → 14 px. Commitado direto. |
| — | **Fila terminada** | todas as telas, modais e a caixa de diálogo global seguem o padrão; o teste `padraoControles.test.js` trava cada uma |
